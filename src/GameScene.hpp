...
public:
    ...
    void start() {
        // è¿›å…¥å€’è®¡æ—¶é˜¶æ®µï¼Œä¸ç«‹åˆ»å¼€å§‹éŸ³ä¹?
        m_phase = Phase::Preparing;
        m_countdownTimer = 0.0f;
        m_finished = false;
    ...
    void update(float deltaTime) {
        // å€’è®¡æ—¶é˜¶æ®µï¼šç´¯ç§¯æ—¶é—´ï¼Œåˆ°ç‚¹å¯åŠ¨éŸ³ä¹?
        if (m_phase == Phase::Preparing) {
            m_countdownTimer += deltaTime;
            if (m_countdownTimer >= COUNTDOWN_DURATION) {
                m_game.startGame();  // æ’­æ”¾éŸ³ä¹ï¼Œè¿›å…? Playing çŠ¶æ€?
                m_phase = Phase::Playing;
            }
            // å€’è®¡æ—¶æœŸé—´ä¸è¿è¡Œæ¸¸æˆé€»è¾‘
        } else if (m_phase == Phase::Playing && m_game.isPlaying()) {
            // å…ˆæ•è·æŒ‰é”®äº§ç”Ÿçš„åˆ¤å®šï¼ˆhandleEvent ä¸­å·²è®¾ç½®ï¼‰ï¼Œå†è¿è¡? game update
            processJudgmentDisplay();
            m_game.update(m_game.getMusicTime());
            // å†æ•è? auto-miss ç­? update å†…äº§ç”Ÿçš„åˆ¤å®š
            processJudgmentDisplay();
        }
        ...
        // æ›´æ–°æ¯ä¸ªè½¨é“çš„åˆ¤å®šæ˜¾ç¤ºè®¡æ—¶å™¨
        for (int i = 0; i < 4; i++) {
...
private:
    ...
    void setupUI() {
        ...
        // åˆ¤å®šçº¿ä¸‹ç§»åˆ°åº•éƒ¨ï¼Œæœ€å¤§åŒ–è½¨é“é•¿åº¦
        m_trackY = 640.0f;
        m_trackTop = 10.0f;
        m_trackHeight = m_trackY - m_trackTop;
        m_noteSpeed = 150.0f;
        // å¯è§æ—¶é—´: 630/150 = 4.2ç§’ï¼ˆæ¯”ä¹‹å‰?2.7ç§’å¤š56%ï¼?
        m_visibleAhead = 4.5f;
    ...
    void drawNotes() {
        ...
        for (const auto& note : notes) {
            ...
            // ¸ù¾İÒô·ûÀàĞÍÉèÖÃÑÕÉ«£¬²»ÔÙÇø·Ö¹ìµÀ
            sf::Color noteColor;
            if (note.type == 1) {
                noteColor = sf::Color(100, 180, 255);  // µ­À¶É«
            } else {
                noteColor = sf::Color(255, 220, 100);  // Ó«»ÆÉ«
            }

            // ---- tap Òô·û ----
            if (note.type != 1) {
                // ÒÑ±»»÷ÖĞ£º¶³½áÔÚÅĞ¶¨ÏßÎ»ÖÃ£¬¶ÌÔİÏûÊ§
                if (note.hit) {
                ...
                noteShape.setFillColor(noteColor);  // Ê¹ÓÃĞÂÑÕÉ«
                m_window.draw(noteShape);
                continue;
            }
    ...
    void drawJudgment() {
        for (int i = 0; i < 4; i++) {
            if (m_trackJudgmentTimer[i] > 0.0f && !m_trackJudgmentText[i].empty()) {
                ...
                // æ¯ä¸ªè½¨é“åˆ¤å®šæ˜¾ç¤ºåœ¨å¯¹åº”è½¨é“çš„åˆ¤å®šçº¿ä¸Šæ–?
